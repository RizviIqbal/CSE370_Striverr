...
/* Fetch ACTIVE projects this freelancer is hired on */
$stmt = $conn->prepare("
  SELECT 
    p.project_id,
    p.title,
    ...
    COALESCE((
      ...
      WHERE m.project_id = p.project_id 
        AND (m.status IN ('approved','released','paid','done','completed'))
    ),0) AS done_ms
  FROM projects p
  JOIN users u ON u.user_id = p.client_id
  WHERE p.hired_freelancer_id = ? AND p.status IN ('active','in_progress')
...
<head>
<meta charset="UTF-8">
<title>Striverr | Active Projects (Freelancer)</title>
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .chip{background:#0b1a28; border:1px solid #1f4259; color:#bfe9ff; font-size:12px; padding:6px 10px; border-radius:14px}
  .desc{color:#d7ecff; opacity:.9; font-size:13px; line-height:1.5; max-height:72px; overflow:hidden}

  .progress{background:#0c1a29; border:1px solid #1e3950; border-radius:12px; overflow:hidden; position:relative; height:12px;}
  .bar{
...
<body>
  ...
  <div class="filters">
    <input class="input" id="searchBox" type="text" placeholder="Search by title or client…">
  ...
  <?php if (empty($rows)): ?>
    <div class="empty">
      <div style="font-size:22px; font-weight:700; margin-bottom:8px;">You have no active projects yet</div>
      <div style="margin-bottom:16px;">Apply to projects and get hired to see them here.</div>
      <a class="btn primary" style="display:inline-block; max-width:220px;" href="browse_projects.php">
        <i class="fa fa-magnifying-glass"></i> Browse Projects
      </a>
    </div>
  <?php else: ?>
      ...
      ?>
        <div class="card"
          ...
          <div class="row-actions">
            <a class="btn primary" href="project_details.php?project_id=<?= (int)$p['project_id'] ?>">
              <i class="fa fa-folder-open"></i> Open Details
            </a>
